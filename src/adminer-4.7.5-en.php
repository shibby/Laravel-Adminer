...
adminer_cookie($B,$Y,$ve=2592000){global$ba;return
...
adminer_redirect($xe,$Le=null){if($Le!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($xe!==null?$xe:$_SERVER["REQUEST_URI"]))][]=$Le;}if($xe!==null){if($xe=="")$xe=".";header("Location: $xe");exit;}}function
...
false;}if($Bg)adminer_redirect($xe,$Le.$zh);return
true;}function
...
adminer_view($B){global$g;return
...
adminer_view($B){$J=get_rows('SELECT text "select" FROM user_views WHERE view_name = '.q($B));return
reset($J);}function
collations(){return
array();}function
information_schema($l){return
...
false;}}foreach($aj
as$Q){$B=($Xh==DB?table("copy_$Q"):idf_escape($Xh).".".table($Q));$Zi=adminer_view($Q);if(($_POST["overwrite"]&&!queries("DROP VIEW IF EXISTS $B"))||!queries("CREATE VIEW $B AS $Zi[select]"))return
false;}return
true;}function
trigger($B){if($B=="")return
...
adminer_cookie("adminer_settings",http_build_query($ph+adminer_settings()));}function
...
';}elseif(isset($_GET["dump"])){$a=$_GET["dump"];if($_POST&&!$n){$Db="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$y)$Db.="&$y=".urlencode($_POST[$y]);adminer_cookie("adminer_export",substr($Db,1));$S=array_flip((array)$_POST["tables"])+array_flip((array)$_POST["data"]);$Ic=dump_headers((count($S)==1?key($S):DB),(DB==""||count($S)>1));$Zd=preg_match('~sql~',$_POST["format"]);if($Zd){echo"-- Adminer $ia ".$ec[DRIVER]." dump\n\n";if($x=="sql"){echo"SET NAMES utf8;
SET time_zone = '+00:00';
...
as$B=>$o){$X=process_input($o);if($X!==false&&$X!==null)$N[idf_escape($B)]=$X;}if($Ji){if(!$N)adminer_redirect($xe);queries_redirect($xe,'Item has been updated.',$m->update($a,$N,$xg,!$Ei));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$G=$m->insert($a,$N);$oe=($G?last_id():0);queries_redirect($xe,sprintf('Item%s has been inserted.',($oe?" $oe":"")),$G);}}}$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($p
...
as$l){if(count($k)==1||$l!=""){if(!create_database($l,$I["collation"]))$Jh=false;$ne=$l;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($ne),'Database has been created.',$Jh);}}else{if(!$I["collation"])adminer_redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($B).(preg_match('~^[a-z0-9_]+$~i',$I["collation"])?" COLLATE $I[collation]":""),substr(ME,0,-1),'Database has been altered.');}}page_header(DB!=""?'Alter database':'Create database',$n,array(),h(DB));$pb=collations();$B=DB;if($_POST)$B=$I["name"];elseif(DB!="")$I["collation"]=db_collation(DB,$pb);elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$ld){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\.\*)?~',$ld,$A)&&$A[1]){$B=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
adminer_redirect($_);}}page_header($_GET["ns"]!=""?'Alter schema':'Create schema',$n);if(!$I)$I["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
adminer_redirect($_);}page_header($fa!=""?'Alter sequence'.": ".h($fa):'Create sequence',$n);if(!$I)$I["name"]=$fa;echo'
<form action="" method="post">
